...
//                            : FileImage(_image)
  ...
  void _register() async {
    await _auth
      ...
      firebaseUser.getIdToken(refresh: false).then((value) {
        currentUser = new UserModel(auth.user.email, _nameController.text,
            dropdownValue, auth.user.uid, "", value.token);
        print('User Token: ${currentUser.userToken}');
      });
    }).catchError((error) {
    ...
    if (currentUser != null) {
      if (currentUser.role == "RELEVANT AUTHORITIES") {
        ...
        writeDataToDataBase(currentUser).then((s) {
          ...
          Route newRoute = MaterialPageRoute(builder: (_) => ClientStoreHome());
          Navigator.pushReplacement(context, newRoute);
        });
  ...
  Future writeDataToDataBase(UserModel currentUser) async {
    ...
    await Tswana_Search.sharedPreferences
        .setStringList(Tswana_Search.userFriendList, ['garbageValue']);
    await Tswana_Search.sharedPreferences
